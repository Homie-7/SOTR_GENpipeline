...
| S9-STU-L | `prompts/S9-STU-L.txt` v1, NBP reference-led, new composition | `@loc_SOTR_studio_L_s9_v1` | **confirmed 2026-09-22** — the canvas re-staged as a single wall. Waits on: the room master regenerated under the night-light prompt (v3) and approved |
...
| ~~`@loc_SOTR_studio_room_s9_v1` v2~~ | Studio | ROOM (reference only, never projected) | n/a | **SUPERSEDED 2026-09-22** | Soul Cinema 21:9 2k, batch 4, var 3 of 4. 2528x1088. Was approved on daylight; the light source is now changed to night/candlelight (see LOG.md 2026-09-22 — a Gantt task in our own workstream and the sound brief both describe the studio scene as "two in the morning" / "moonlit window", neither of which is in the script itself). **File kept at `SOTR_MEDIA/plates/studio/loc_SOTR_studio_room_s9_v1.png` for reference only — do not use as a wall-prompt source.** Must be regenerated from `prompts/S9-STU-ROOM.txt` v3 and re-approved before any wall references it. Canvas proportion note (~1.66:1 vs real 1.46:1) still applies to the regeneration |
